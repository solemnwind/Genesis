...
def esim(x_end, current_image, previous_image, delta_time, crossings,
    ...
    # Pre-allocate temporary arrays for parallel writing
    event_counts = np.zeros(x_end, dtype=np.int32)
    event_data = np.zeros((x_end, max_spikes, 4), dtype=np.float64)  # x,y,timestamp,polarity
    ...
    for x in prange(x_end):
        ...
        time_step = delta_time / spike_nums if spike_nums > 0 else 0
        
        for i in range(spike_nums):
            if event_counts[x] >= max_spikes:
                break
                
            event_data[x, event_counts[x], 0] = x % n_pix_row
            event_data[x, event_counts[x], 1] = x // n_pix_row
            event_data[x, event_counts[x], 2] = np.round(current_time * 1e-6, 6)
            event_data[x, event_counts[x], 3] = 1 if pol > 0 else -1
            
            event_counts[x] += 1
            current_time += time_step

    # Sequential compaction of events
    total_events = np.sum(event_counts)
    if total_events > max_events_per_frame:
        total_events = max_events_per_frame
        
    for x in range(x_end):
        for i in range(min(event_counts[x], max_events_per_frame - count)):
            output_events[count].x = event_data[x, i, 0]
            output_events[count].y = event_data[x, i, 1]
            output_events[count].timestamp = event_data[x, i, 2]
            output_events[count].polarity = event_data[x, i, 3]
            count += 1
